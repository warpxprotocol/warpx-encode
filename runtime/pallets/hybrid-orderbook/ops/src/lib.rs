...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn migrate_to_new_account(
            ...
            if let Some((depositor, deposit)) =
                T::PoolAssetsRefund::deposit_held(info.lp_token(), prior_account.clone())
            {
                T::DepositAsset::mint_into(&depositor, deposit + deposit_asset_ed)?;
                T::PoolAssets::touch(info.lp_token(), &new_account, &depositor)?;
            }
            ...
            if let Some((depositor, deposit)) =
                T::PoolAssetsRefund::deposit_held(info.lp_token(), prior_account.clone())
            {
                T::PoolAssetsRefund::refund(info.lp_token(), prior_account.clone())?;
                T::DepositAsset::burn_from(
                    &depositor,
                    deposit + deposit_asset_ed,
                    Preservation::Expendable,
